...
export default function LoginPage() {
  ...
  const [namaWaiting,     setNamaWaiting]     = useState('');
  ...
  return (
    <div style={{
      minHeight:'100vh',
      background:'linear-gradient(135deg,#0F3460 0%,#1B4F8A 50%,#1565C0 100%)',
        ...
        <div style={{ background:'#fff', borderRadius:24, overflow:'hidden', boxShadow:'0 24px 80px rgba(0,0,0,0.25)' }}>
          ...
          {waitingApproval ? (
            <div style={{ padding:'40px 32px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:20, fontWeight:700, color:'#1A2332', marginBottom:8 }}>
                Pendaftaran Berhasil!
              </div>
              <div style={{ fontSize:14, color:'#718096', marginBottom:24, lineHeight:1.8 }}>
                Halo <strong style={{ color:'#1A2332' }}>{namaWaiting}</strong>, akun Anda sudah terdaftar.<br/>
                Silakan tunggu persetujuan dari <strong style={{ color:'#1B5EA0' }}>Admin / Kepala Desa</strong><br/>
                sebelum bisa login ke sistem.
              </div>
              <div style={{ background:'#FAEEDA', border:'1px solid #F5CE8A', color:'#A0621B', borderRadius:12, padding:'14px 16px', fontSize:13, marginBottom:28, lineHeight:1.7 }}>
                ⚠️ Akun Anda <strong>belum aktif</strong>. Admin akan mengaktifkan akun Anda setelah melakukan verifikasi data.
              </div>
              <div style={{ background:'#EAF3DE', border:'1px solid #B8D98C', color:'#2D6A0F', borderRadius:12, padding:'12px 16px', fontSize:13, marginBottom:28 }}>
                ✅ Jika sudah diaktifkan, silakan login menggunakan username dan password yang sudah didaftarkan.
              </div>
              <button
                onClick={() => { setWaitingApproval(false); setMode('login'); }}
                style={{ width:'100%', padding:'14px', fontSize:16, fontWeight:700, background:'linear-gradient(135deg,#1B5EA0,#1565C0)', color:'#fff', border:'none', borderRadius:12, cursor:'pointer', boxShadow:'0 4px 14px rgba(27,94,160,0.35)' }}>
            ...
            <>
              ...
              <div style={{ display:'flex', borderBottom:'1.5px solid #E2E8F0' }}>
                {[{id:'login',label:'🔑 Masuk'},{id:'register',label:'📝 Daftar'}].map(t=>(
                  <button key={t.id}
                    onClick={()=>{ setMode(t.id); setLoginError(''); setRegError(''); }}
                ...
                {mode==='register' && (
                  <form onSubmit={handleRegister}>
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                        {ROLE_OPTIONS.map(r=>(
                          <button type="button" key={r.value}
                            onClick={()=>setRegForm({...regForm,role:r.value,jabatan:r.value==='perangkat_desa'?JABATAN_PERANGKAT[0]:'Kepala Desa'})}
                    ...
                    <div style={{ marginBottom:14 }}>
                      ...
                      <input value={regForm.nama} onChange={e=>setRegForm({...regForm,nama:e.target.value})}
                        placeholder="Nama sesuai identitas" style={inputStyle}
                        onFocus={e=>e.target.style.borderColor='#1B5EA0'}
                        onBlur={e=>e.target.style.borderColor='#CBD5E1'} />
                    </div>
                    ...
                    <div style={{ marginBottom:14 }}>
                      ...
                      {regForm.role==='kepala_desa'
                        ...
                        : <select value={regForm.jabatan} onChange={e=>setRegForm({...regForm,jabatan:e.target.value})}
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
                      <div>
                        ...
                        <input value={regForm.nip} onChange={e=>setRegForm({...regForm,nip:e.target.value})}
                          placeholder="NIP atau NIK" style={inputStyle}
                          onFocus={e=>e.target.style.borderColor='#1B5EA0'}
                          onBlur={e=>e.target.style.borderColor='#CBD5E1'} />
                      </div>
                      <div>
                        <label style={{ display:'block', fontSize:14, fontWeight:600, color:'#4A5568', marginBottom:6 }}>No. HP</label>
                        <input value={regForm.noHp} onChange={e=>setRegForm({...regForm,noHp:e.target.value})}
                    ...
                    <div style={{ marginBottom:14 }}>
                      <label style={{ display:'block', fontSize:14, fontWeight:600, color:'#4A5568', marginBottom:6 }}>
                        Username <span style={{ color:'#C0392B' }}>*</span>
                      </label>
                      <input value={regForm.username}
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:20 }}>
                      ...
                      <div>
                        <label style={{ display:'block', fontSize:14, fontWeight:600, color:'#4A5568', marginBottom:6 }}>
                          Ulangi Password <span style={{ color:'#C0392B' }}>*</span>
                        </label>
                        <input type="password" value={regForm.konfirmasi}
