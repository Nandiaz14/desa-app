...
export function Modal({ show, onClose, title, children, width = 560 }) {
  ...
  return (
    <div
      style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-start', justifyContent:'center', zIndex:1000, padding:'32px 16px', overflowY:'auto', backdropFilter:'blur(4px)' }}
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div style={{ background:'#fff', borderRadius:20, width:'100%', maxWidth:width, boxShadow:'0 20px 60px rgba(0,0,0,0.2)', marginBottom:32, animation:'slideUp 0.25s ease' }}>
...
export function ConfirmModal({ show, onClose, onConfirm, title, message, confirmLabel='Hapus', confirmVariant='danger', icon='🗑' }) {
  if (!show) return null;
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1100, padding:16, backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#fff', borderRadius:20, padding:'32px 28px', width:'100%', maxWidth:380, boxShadow:'0 20px 60px rgba(0,0,0,0.25)', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onClose} style={{ flex:1, padding:'12px', fontSize:14, fontWeight:600, background:'#F1F5F9', color:'#4A5568', border:'1.5px solid #CBD5E1', borderRadius:12, cursor:'pointer' }}>Batal</button>
          <button onClick={()=>{ onConfirm(); onClose(); }} style={{ flex:1, padding:'12px', fontSize:14, fontWeight:700, background:confirmVariant==='danger'?'#C0392B':'#1B5EA0', color:'#fff', border:'none', borderRadius:12, cursor:'pointer' }}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
...
export function Skeleton({ width='100%', height=16, borderRadius=8, style }) {
  return (
    <div style={{ width, height, borderRadius, background:'linear-gradient(90deg,#F1F5F9 25%,#E2E8F0 50%,#F1F5F9 75%)', backgroundSize:'200% 100%', animation:'shimmer 1.5s infinite', ...style }} />
  );
}
...
export function Pagination({ page, total, perPage=10, onChange }) {
  ...
  const btnStyle = (active) => ({ width:36, height:36, borderRadius:8, border:active?'none':'1px solid #E2E8F0', background:active?'#1B5EA0':'#fff', color:active?'#fff':'#4A5568', cursor:'pointer', fontFamily:'inherit', fontSize:13, fontWeight:active?700:400, display:'flex', alignItems:'center', justifyContent:'center' });
  return (
...
export function Btn({ children, onClick, variant='default', style, disabled, type='button', size='md' }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled}
      style={{ ...v, borderRadius:10, padding, fontSize, fontWeight:600, cursor:disabled?'not-allowed':'pointer', opacity:disabled?0.5:1, fontFamily:'inherit', transition:'all 0.15s', display:'inline-flex', alignItems:'center', gap:6, boxShadow:variant==='primary'?'0 2px 8px rgba(27,94,160,0.3)':'none', ...style }}>
      {children}
    </button>
  );
}
    ...
    <div className="tab-bar">
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>onChange(t.id)}
          ...
          {t.label}
          {t.badge?(<span style={{ background:'#C0392B', color:'#fff', borderRadius:12, fontSize:11, padding:'2px 7px', fontWeight:700 }}>{t.badge}</span>):null}
        </button>
      ))}
    </div>
  );
...
export function SearchBar({ value, onChange, placeholder }) {
  return (
    <div style={{ position:'relative', marginBottom:12 }}>
      ...
      <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||'Cari...'}
        style={{ width:'100%', border:'1.5px solid #CBD5E1', borderRadius:10, padding:'11px 40px 11px 44px', fontSize:14, fontFamily:'inherit', background:'#fff', outline:'none', boxSizing:'border-box' }}
        onFocus={e=>e.target.style.borderColor='#1B5EA0'}
        onBlur={e=>e.target.style.borderColor='#CBD5E1'}
      />
      {value && (
        <button onClick={()=>onChange('')} style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'#E2E8F0', border:'none', borderRadius:'50%', width:22, height:22, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
      )}
    </div>
  );
}